...
def fitness(trial, guesses):
	# fitness without being able to ask the master the score
	# only based on previous guesses

	# result = score(trial, guesses[-1][0]) # just the last guess
	
	# or all previous guesses 
	sum_black_differences = 0  # Sum of well placed colors
	sum_white_differences = 0  # Sum of wrong placed colors

	for prev_guess in guesses:
		score_obtained = score(trial, guesses[i][0])
		sum_black_differences += score_obtained[0]
		sum_white_differences += score_obtained[1]

	# Final score
	fitness_score = sum_black_differences + sum_white_differences
